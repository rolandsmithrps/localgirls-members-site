...
function useAuthGate(){ useEffect(()=>{ if(typeof window!=='undefined' && sessionStorage.getItem('lg-auth')!=='ok'){ window.location.href='/login'; } },[]); }
export default function Members(){ useAuthGate(); const [models,setModels]=useState<Model[]>([]);
const refresh=async()=>{ const r=await fetch('/api/models',{cache:'no-store'}); const d:Model[]=await r.json(); setModels([...d].sort((a,b)=> a.slug==='Taylored2U' ? -1 : b.slug==='Taylored2U' ? 1 : 0)); };
